...
function MonthCard({ month, pct, trades, wins }) {
  ...
  return (
    <div onMouseEnter={handleEnter} onMouseLeave={handleLeave} style={{
      ...
      <div style={{ fontSize: '0.72rem', color: '#8A9B8D', textTransform: 'uppercase', letterSpacing: '0.08em' }}>{month}</div>
      <div style={{ fontFamily: 'Montserrat, sans-serif', fontSize: '2rem', fontWeight: 800, color: '#8EF43A', letterSpacing: '-0.045em', lineHeight: 1 }}>{pct}</div>
      <div style={{ fontSize: '0.78rem', color: '#8A9B8D' }}>{trades} trades · {wins} wins</div>
      <div style={{
        height: '100px', background: 'rgba(3,9,5,0.76)', borderRadius: '10px',
        border: '1px dashed rgba(120,245,27,0.15)',
        display: 'flex', alignItems: 'center', justifyContent: 'center',
        fontSize: '0.75rem', color: '#8A9B8D',
      }}>Screenshot</div>
    </div>
  )
}
...
function LiveCard({ pair, rr, date }) {
  ...
  return (
    <div onMouseEnter={handleEnter} onMouseLeave={handleLeave} style={{
      ...
      <div style={{
        height: '160px', background: 'rgba(3,9,5,0.76)',
        borderBottom: '1px solid rgba(120,245,27,0.08)',
        display: 'flex', alignItems: 'center', justifyContent: 'center',
        fontSize: '0.75rem', color: '#8A9B8D',
      }}>Trade Screenshot</div>
      <div style={{ padding: '20px' }}>
        <div style={{ fontFamily: 'Montserrat, sans-serif', fontWeight: 700, fontSize: '0.95rem', color: '#F5F3FF', marginBottom: '6px', letterSpacing: '-0.02em' }}>{pair}</div>
        <div style={{ fontSize: '0.8rem', color: '#8EF43A', fontWeight: 600, marginBottom: '4px' }}>{rr}</div>
        <div style={{ fontSize: '0.78rem', color: '#8A9B8D' }}>{date}</div>
      </div>
    </div>
  )
}
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        marginBottom: '48px', flexDirection: isMobile ? 'column' : 'row',
        maxWidth: isMobile ? '300px' : 'none', margin: isMobile ? '0 auto 48px' : '0 0 48px',
      }}>
        <button style={tabStyle('monthly')} onClick={() => setActiveTab('monthly')}>Monthly Returns</button>
        <button style={tabStyle('live')} onClick={() => setActiveTab('live')}>Live Trading</button>
      </div>
